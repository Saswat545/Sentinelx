...
const normalizeItem = (it: any) => (typeof it === 'string' ? { image: it, alt: '', title: '', amount: '', date: '', description: '' } : it);
  ...
  return (
    <div ref={rootRef} className={`depth-carousel ${className}`.trim()} style={{ '--dc-perspective': `${perspective}px` } as React.CSSProperties} role="group" aria-roledescription="carousel" aria-label="Depth carousel" tabIndex={0} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerEnd} onPointerCancel={onPointerEnd} onKeyDown={onKeyDown}>
      <div className="depth-carousel__stage">
        {data.map((item, i) => (
          <div key={i} ref={el => { cardRefs.current[i] = el; }} className="depth-carousel__card" style={{ width: cardWidth, height: cardHeight, borderRadius: radius }} aria-roledescription="slide" aria-label={`${i + 1} of ${count}`} aria-hidden={active !== i} onClick={() => onCardClick(i)}>
            ...
            {item.title && (
              <div className="depth-carousel__info">
                {item.amount && <span className="depth-carousel__amount">{item.amount}</span>}
                <h3 className="depth-carousel__title">{item.title}</h3>
                {item.date && <span className="depth-carousel__date">{item.date}</span>}
